...
const addValues = (arr) => {
  return arr.reduce((increase, current) => increase + current, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((sum, purchase) => sum + purchase.purchasePrice, 0);
};
...
const returnNames = (arr) => {
  return arr.reduce((names, starWars) => {
    names.push(starWars.name);
    return names;
  }, []);
};
...
const reversedString = (str) => {
  return str.split('').reduce((reverse, name) => {
    return name + reverse;
  }, '');
};
